...
int subtree_size(struct bst_node* node) {
  ...
  return subtree_size(node->left) + subtree_size(node->right) + 1;
}
/*
...
int check_subtree_path_sum(struct bst_node* node, int sum) {
  if(node == NULL) return 0;
  if(node->left == NULL && node->right == NULL) return (sum == node->val);
  return check_subtree_path_sum(node->left, sum - node->val) || // Check left subtree recursively
      check_subtree_path_sum(node->right, sum - node->val);     // Check right subtree recursively
}
/*
...
int bst_path_sum(int sum, struct bst* bst) {
  assert(bst);
  return check_subtree_path_sum(bst->root, sum);
}

struct bst_node* get_last_node(struct bst_node* node) {
...
struct bst_iterator* bst_iterator_create(struct bst* bst) {
  assert(bst);
  struct bst_iterator* iter = malloc(sizeof *iter);
  assert(iter);

  // Create the path stack
  iter->path = stack_create();
  struct bst_node* top = malloc(sizeof *top);
  top->right = bst->root;
  stack_push(iter->path, top);
  
  // Store last node
  iter->last_node = get_last_node(bst->root);
  if(iter->last_node == NULL) {
    iter->last_node = stack_top(iter->path);
  }
...
void bst_iterator_free(struct bst_iterator* iter) {
  assert(iter);
  // Need to free allocated top node
  struct bst_node* last = NULL;
  while(!stack_isempty(iter->path)) {
    last = stack_pop(iter->path);
  }
  free(last);
  // Free stack and iterator
  stack_free(iter->path);
  free(iter);
}
...
int bst_iterator_has_next(struct bst_iterator* iter) {
  return (stack_top(iter->path) != iter->last_node);
}
...
int bst_iterator_next(struct bst_iterator* iter) {
  assert(iter);
  struct bst_node* cur = stack_top(iter->path);

  // Fist try to climb down right branch
  if(cur->right) {
    // Climb down tree to find next - the leftmost child of right subtree
    struct bst_node* next = cur->right;
    while(next) {
      stack_push(iter->path, next);
      next = next->left;
    }
    next = stack_top(iter->path);
    return next->val;
  }

  // Otherwise climb up tree to find common ancestor of cur and next
  struct bst_node* parent;
  int flag = 1;
  while(flag) {
    cur = stack_pop(iter->path);
    parent = stack_top(iter->path);
    flag = (parent->right == NULL || parent->right == cur);
  }
  return parent->val;
}
